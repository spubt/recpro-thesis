...
public class EnvironmentalParameterEvaluationExecutionService {
    private final ApiService apiService;



    public List<EnvironmentalParameterConfiguration> getEnvironmentalParametersByActivity(Activity activity) {
        return apiService.getRestClient().get().uri(RECPRO_MODELING_ERC_DATA_ENVIRONMENTAL_PARAMETER_CONFIGURATION + GET_BY_ACTIVITY_ID + activity.getId()).retrieve().body(new ParameterizedTypeReference<>(){});
    }

    public Object fillParameters(String activityId, String environmentalParameterId, String event) {
        ...
        try {
            ...
            return process.get(activityId).get(environmentalParameterId);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
